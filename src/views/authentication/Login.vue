        ...
        >
          ...
          <b-form-group>
            ...
            <b-input-group
              ...
              :class="passwordError ? 'is-invalid' : null"
            >
              <b-form-input
                id="password"
                v-model="password"
...
export default {
  ...
  computed: {
    passwordToggleIcon() {
      return this.passwordFieldType === 'password' ? 'Eye' : 'EyeOff'
    },
    isFormInvalid() {
      return !this.userEmail || !this.password || this.emailError || this.passwordError
    },
  },
  methods: {
    validateEmail() {
      ...
      try {
        ...
        if (!accessToken) {
          this.$toast({
            component: ToastificationContent,
            position: 'top-right',
            props: { title: 'No Google access token provided', icon: 'AlertTriangle', variant: 'warning' },
          })
          this.blockButton = false
          return
        }
        ...
        if (token && user) {
          ...
          this.$toast({
            component: ToastificationContent,
            position: 'top-right',
            props: { title: 'Invalid Google login response', icon: 'X', variant: 'danger' },
          })
        }
      } catch (error) {
        this.$toast({
          component: ToastificationContent,
          position: 'top-right',
          props: { title: 'Google login failed', icon: 'X', variant: 'danger' },
        })
      } finally {
        this.blockButton = false
      }
