        ...
        <b-container align-h="around" v-for="word in words" :key="word.id">
          <b-row class="justify-content-center" v-if="word.show" >
            ...
            >
                ...
                >
                  <b-list-group-item v-for="translate in word.checkTranslate" :key="translate"
...
export default {
  ...
  beforeUpdate() {
    this.$nextTick(()=> {
      const audioButtons = document.getElementsByClassName('audio_play_button');
      audioButtons.forEach((element) => {
        addEventListener('click', this.playSound2)
        element.classList.add("bi-play-circle");
        // element.innerHTML = '<i class="bi-alarm" style="font-size: 2rem; color: cornflowerblue;"></i>'
      })
    })
  },
  watch: {
  ...
  methods: {
    getWords() {
      this.loading = true
      axios.get(`${process.env.VUE_APP_API_URL}/learning/words_translate`, {
    ...
    check(word, translate, e) {
      ...
      if (!check) {
        ...
        if (word.first === 0) {
          word.first = 1
          axios.get(`${process.env.VUE_APP_API_URL}/learning/change_status?check=wt&id=${word.word_id}`, {
            headers: { Authorization: `Bearer ${useJwt.getToken()}` },
          }).then(response => {
            this.loading = false
          }).catch(error => {
